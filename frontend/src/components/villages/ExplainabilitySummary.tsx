...
export const ExplainabilitySummary: React.FC<ExplainabilitySummaryProps> = ({
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-lg p-5 mb-6 shadow-sm">
      <div className="flex items-center justify-between mb-4">
        ...
        <span className="text-xs font-mono text-slate-400">Deterministic Backend Trace</span>
      </div>

      {/* Progressive Flow Pipeline: Village -> Exposure/Vulnerability -> Risk -> Priority */}
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
        {/* Stage 1: Village Identity */}
        <div className="p-3 bg-slate-950/80 border border-slate-800 rounded relative">
          <div className="text-xs font-mono text-slate-500 uppercase tracking-wider mb-1">
            1. Settlement
          </div>
          <div className="font-semibold text-slate-200 text-sm truncate" title={habitation.name}>
            {habitation.name}
          </div>
      ...
      <div className="mt-3 text-xs text-slate-500 font-mono flex items-center justify-between flex-wrap gap-2">
