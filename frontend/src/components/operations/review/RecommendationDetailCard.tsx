...
export const RecommendationDetailCard: React.FC<RecommendationDetailCardProps> = ({
  ...
  return (
    <Card className="border-border-subtle bg-surface-panel shadow-sm overflow-hidden">
      ...
      <CardHeader className="p-5 border-b border-border-subtle bg-surface-elevated">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
          <div>
            <div className="flex items-center gap-2 flex-wrap mb-1.5">
              ...
              <Badge variant="outline" size="sm" className="font-mono text-sky-700 dark:text-sky-400 border-sky-300 dark:border-sky-600/50">
            ...
            <CardDescription className="text-xs text-text-muted mt-1 flex items-center gap-2 flex-wrap">
              <span className="inline-flex items-center gap-1">
                <Cpu className="h-3 w-3 text-emerald-600 dark:text-emerald-400" />
                <strong className="text-text-secondary">Engine:</strong> {dossier.source_engine}
              </span>
              <span className="text-text-muted">•</span>
              <span className="inline-flex items-center gap-1">
                <Layers className="h-3 w-3 text-sky-600 dark:text-sky-400" />
                <strong className="text-text-secondary">Profile:</strong> {dossier.region_profile_id}
              </span>
            </CardDescription>
          </div>
        ...
        <div className="mt-4 rounded-lg border border-amber-300 dark:border-amber-800/60 bg-amber-50 dark:bg-amber-950/30 p-3 text-xs text-amber-900 dark:text-amber-200/90 leading-relaxed flex items-start gap-2.5">
          <ShieldAlert className="h-4 w-4 text-amber-600 dark:text-amber-400 shrink-0 mt-0.5" />
          <div>
            <div className="font-semibold text-amber-950 dark:text-amber-300 font-mono">
              Statutory Rule 12 Protocol — Mandatory Officer Authorization
            </div>
            <p className="mt-0.5 text-[11px] text-amber-800 dark:text-amber-200/80">
      ...
      <CardContent className="p-5 space-y-6">
        ...
        <div className="rounded-lg border border-sky-200 dark:border-sky-900/60 bg-sky-50 dark:bg-sky-950/20 p-4">
          <div className="flex items-center gap-2 text-xs font-semibold text-sky-900 dark:text-sky-300 mb-1.5">
        ...
        {/* Executive Narrative */}
        <div>
          <h4 className="text-xs font-semibold text-text-primary uppercase tracking-wider mb-2 font-mono">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
          {dossier.metrics.map((m, idx) => (
            <div
              key={idx}
              className="rounded-lg border border-border-subtle bg-surface-elevated p-3 text-center"
            >
        ...
        {dossier.type === "relocation_plan" && dossier.relocation_payload && (
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <h4 className="text-xs font-semibold text-text-primary uppercase tracking-wider font-mono flex items-center gap-2">
            ...
            <div className="overflow-x-auto rounded-lg border border-border-subtle bg-surface-panel">
              <table className="w-full text-left text-xs">
                <thead className="border-b border-border-subtle bg-surface-elevated text-text-muted font-mono text-[11px]">
                  <tr>
                ...
                <tbody className="divide-y divide-border-subtle text-text-secondary">
                  {dossier.relocation_payload.assignments.map((assignment) => {
                    ...
                    return (
                      <tr key={String(assignment.village_id)} className="hover:bg-surface-elevated">
                        ...
                        <td className="py-2.5 px-3">
                          {isAssigned ? (
                            ...
                            <span className="text-rose-600 dark:text-rose-400 italic">None (Capacity Exhausted)</span>
                          )}
                        </td>
          ...
          <div className="space-y-3">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
              <div className="rounded-lg border border-border-subtle bg-surface-elevated p-3">
                <div className="text-[11px] text-text-muted font-mono">Rainfall Surge Multiplier</div>
                <div className="text-base font-bold text-amber-700 dark:text-amber-300 font-mono mt-1">
                  {dossier.scenario_payload.parameters.rainfall_multiplier}x
                </div>
                <div className="text-[10px] text-text-muted mt-0.5">Heavy precipitation baseline</div>
              </div>

              <div className="rounded-lg border border-border-subtle bg-surface-elevated p-3">
                <div className="text-[11px] text-text-muted font-mono">Transit Road Blockage</div>
                <div className="text-base font-bold text-rose-700 dark:text-rose-300 font-mono mt-1">
                  {dossier.scenario_payload.parameters.road_blockage_percentage}%
                </div>
                <div className="text-[10px] text-text-muted mt-0.5">Highway route occlusion</div>
              </div>

              <div className="rounded-lg border border-border-subtle bg-surface-elevated p-3">
                <div className="text-[11px] text-text-muted font-mono">Flood Inundation Increase</div>
                <div className="text-base font-bold text-sky-700 dark:text-sky-300 font-mono mt-1">
                  +{dossier.scenario_payload.parameters.flood_hazard_increase}%
                </div>
                <div className="text-[10px] text-text-muted mt-0.5">River valley hazard delta</div>
              </div>
            </div>

            <div className="rounded-lg border border-border-subtle bg-surface-elevated p-3.5 text-xs">
