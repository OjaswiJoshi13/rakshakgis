...
export const SiteSuitabilityTab: React.FC<SiteSuitabilityTabProps> = ({
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 rounded-lg border border-border-subtle bg-surface-panel p-4 shadow-xs">
        <div className="flex items-center gap-3">
          <div className="flex h-12 w-12 items-center justify-center rounded-lg bg-surface-elevated border border-border-subtle">
          ...
          <div>
            <div className="flex items-center gap-2 mb-1">
              <span className="text-xs font-mono uppercase tracking-wider text-text-muted">
                ...
                <span className="sr-only"> M4-02 Classification</span>
              </span>
              ...
              {suitability.is_eligible ? (
                ...
                <span className="text-[11px] font-mono text-red-600 dark:text-red-400 flex items-center gap-1">
        ...
        {/* Score Display */}
        <div className="flex items-baseline gap-2 rounded-lg bg-surface-elevated border border-border-subtle px-4 py-2">
            ...
            >
              ...
              <div className="text-[11px] font-mono text-text-secondary space-y-0.5">
                <div>
                  ...
                  <span className={gate.passed ? "text-text-primary" : "text-red-600 dark:text-red-400 font-bold"}>
              ...
              {gate.reason && (
                <p className="text-[11px] text-text-muted mt-2 border-t border-border-subtle pt-1.5 line-clamp-2">
                  {gate.reason}
                </p>
              )}
            </div>
      ...
      <div className="space-y-3 rounded-lg border border-border-subtle bg-surface-panel p-4 shadow-xs">
        <div className="flex items-center justify-between border-b border-border-subtle pb-2">
          <h4 className="text-xs font-mono uppercase tracking-wider text-text-muted flex items-center gap-1.5">
            <Scale className="h-3.5 w-3.5 text-primary-600 dark:text-primary-400" />
            <span>Authoritative 9-Criteria Weighted Score Decomposition</span>
          </h4>
          <span className="text-[11px] font-mono text-text-muted">Sum of Weights = 1.00</span>
        </div>
              ...
              {Object.entries(suitability.criteria_scores).map(([key, item]) => {
                ...
                return (
                  <tr key={key} className="hover:bg-surface-subtle transition-colors">
                    ...
                    <td className="py-2.5 px-3 font-mono text-primary-600 dark:text-primary-400 font-bold">
                      +{item.weighted_contribution.toFixed(1)} pts
                    </td>

                    <td className="py-2.5 px-3 text-text-secondary text-xs max-w-sm">
                      {item.description}
                    </td>
                  </tr>
                );
