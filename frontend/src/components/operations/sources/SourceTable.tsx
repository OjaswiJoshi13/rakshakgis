...
export const SourceTable: React.FC<SourceTableProps> = ({
  ...
  if (isLoading) {
    return (
      <div className="rounded-lg border border-border-base bg-surface-raised p-8 text-center shadow-2xs">
        <RefreshCw className="h-6 w-6 text-[#0969da] dark:text-[#2f81f7] animate-spin mx-auto mb-2" />
        <p className="text-xs text-text-muted font-mono">Loading telemetry and freshness diagnostics...</p>
      </div>
    );
  }
  ...
  if (sources.length === 0) {
    return (
      <div className="rounded-lg border border-dashed border-border-base bg-surface-raised p-12 text-center shadow-2xs">
        <Server className="h-8 w-8 text-text-muted mx-auto mb-3" />
        <h3 className="text-sm font-semibold text-text-primary">No Data Sources Match Filter Criteria</h3>
  ...
  return (
    <div className="rounded-lg border border-border-base bg-surface-raised overflow-hidden shadow-2xs">
      <div className="overflow-x-auto">
        <table className="w-full text-left text-xs">
          <thead className="bg-surface-base/80 text-text-muted font-mono border-b border-border-base uppercase text-[10px] tracking-wider">
            <tr>
              ...
              return (
                ...
                >
                  ...
                  <td className="py-3 px-4">
                    <div className="font-semibold text-text-primary group-hover:text-[#0969da] dark:group-hover:text-[#2f81f7] transition-colors">
                      {source.name}
                    </div>
                    <div className="flex items-center gap-2 text-[11px] text-text-muted font-mono mt-0.5">
                      <span>ID: #{source.source_id}</span>
                      <span>•</span>
                      <span className="text-text-muted">{source.provider_id || source.provider}</span>
                    </div>
                  </td>

                  {/* Category */}
                  <td className="py-3 px-3">
                    <Badge variant="outline" size="sm" className="capitalize font-mono text-[11px] text-text-secondary border-border-base">
                  ...
                  <td className="py-3 px-3 font-mono text-[11px] tabular-nums">
                    <div className="text-[#1a7f37] dark:text-[#3fb950]">
                      {source.records_ingested_total.toLocaleString()} synced
                    </div>
                    {source.records_failed_total > 0 ? (
                      <div className="text-[#cf222e] dark:text-[#ff7b72] text-[10px]">
                        {source.records_failed_total} failed
                      </div>
                    ) : (
                      <div className="text-text-muted text-[10px]">0 failed</div>
                    )}
                  </td>
                  ...
                  <td className="py-3 px-3">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <Badge variant="outline" size="sm" className="font-mono text-[10px] uppercase text-text-secondary border-border-base">
                        {source.provider_mode}
                      </Badge>
                      {source.is_synthetic && (
                        <Badge variant="outline" size="sm" className="font-mono text-[10px] text-purple-700 dark:text-purple-400 border-purple-600/40 bg-purple-500/10">
                          SYNTHETIC
                        </Badge>
                      )}
                    </div>
